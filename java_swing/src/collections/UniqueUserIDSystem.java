...
public class UniqueUserIDSystem {
    ...
    public HashSet<String> registeredIDs;

    public UniqueUserIDSystem() {
        registeredIDs = new HashSet<>();

        // printAllIDs 메서드를 테스트를 하기 위해서 샘플 데이터 입력해 보기
        // 샘플 데이터 입력 - 추후 삭제 예정
    ...
    public void printAllIDs() {
        ...
        if (registeredIDs.isEmpty()){
            System.out.println("등록된 사용자 ID가 없습니다");
            return; // 실행에 제어권 반납. 아래를 굳이 실행할 필요가 없기 때문에 리턴을 쓰면 좋다.
        }
